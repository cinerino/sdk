...
function authorizeSeatReservationByEvent(params) {
    return async (eventService, placeOrderService, ownershipInfoService, tokenService) => {
        ...
        if (membershipOwnershipInfos.data.length < 1) {
            throw new Error('you do not have any memberships');
        }

        const membershipOwnershipInfo = membershipOwnershipInfos.data.shift();
        console.log('using my membership...', membershipOwnershipInfo.id, membershipOwnershipInfo.typeOfGood.identifier);

        console.log('authorizing ownershipInfo...');
        const { code } = await ownershipInfoService.authorize({ ownershipInfoId: membershipOwnershipInfo.id });
        console.log('ownershipInfo authorized', code);
